...
\end{align}
	...
	Используя лемму {\ref{lemma:Schur}}:
		...
		\end{bmatrix}<0.
	\end{equation}
	Сопрягая блочно-диагональную матрицу $\textbf{diag}({Q}_1, {I})$, где ${Q}_1 = {P}_1^{-1}$, находим:
	%
	\begin{equation}
...
Для того чтобы избавиться от нелинейности в переменных ${Q}_1{K}_z\T{N}_2\T$  и ${Q}_1 {N}\T{N}_3\T $ используем неравенство Юнга.
	...
	Используя лемму \ref{lemma:Young}, получаем
	
	применяя дополнение Шура {\ref{lemma:Schur}} и вводя замену переменных $\hat{{K}}_z={K}_z{Q}_1$, $\hat{{L}}_z={P}_2{L}_z$, получаем:
	%
	\begin{equation}
