...
\begin{equation}
	...
	\end{cases}
\end{equation}
где матрицы $A_n$,  $B$ и  $С$ являются матрицами состояния, управления и наблюдения соответственно. А матрица ${A}_r$ отвечает за статическую часть матрицы состояния.
...
\begin{align}
	...
	\end{bmatrix}{\zeta}
\end{align}
где матрица $\bigl[ \begin{smallmatrix}  {I} & -{D}_K \\ 0 & {I} \end{smallmatrix} \bigr]$ должна быть невырожденной, котороя она и будет всегда являться, следуя:
\begin{equation}
	\begin{gathered}
		\begin{bmatrix}
			{I} & -{D}_K \\ 0 & {I}
	...
	\end{gathered}
\end{equation}
Подставим \ref{eq:part5_system2} в \ref{eq:part5_system1}:
\begin{align}
	\nonumber
	& \begin{bmatrix}
		{\dot{z}} \\ {\dot{z}}_K 
...
\begin{align}
	...
	\end{bmatrix}.
\end{align}
Используя новые обозначения, перепишем \ref{eq:part5_cl}, как:
\begin{align}
...
\begin{theorem}
	...
	и положительный скаляр $\alpha>0$ такие, что выполняется следующие линейно-матричные неравенства:
	...
	\begin{align}\label{eq: condition}
		...
		\end{bmatrix} > 0.
	\end{align}
	И коэффициенты находятся следующим образом 
	\begin{align}
...
\end{theorem}
\begin{proof}
	Введём кандидата в функцию Ляпунова ${V} = {z}_{cl}\T {P}{z}_{cl} \geq 0$ и её производную:
	\begin{align}
		...
		\leq 0,
	\end{align}
	где система считается квадратичной устойчивой в конечное время тогда и только тогда, когда существует положительно-определённая матрица ${P} = \bigl[ \begin{smallmatrix}  {P}_1 & {P}_2 \\ {P}_2\T & {P}_3 \end{smallmatrix} \bigr]$.
