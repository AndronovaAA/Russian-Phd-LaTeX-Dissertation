...
\section{Метод для нахождения оптимального робастного управления для системы с аддитивными неопределённостями и динамической обратной связью}\label{sec:ch5/sect1}
...
\begin{align}
	...
	\end{bmatrix}{\zeta},
\end{align}
где матрица $\bigl[ \begin{smallmatrix}  {I} & -{D}_K \\ 0 & {I} \end{smallmatrix} \bigr]$ должна быть невырожденной, которой она и будет всегда являться, следуя:
\begin{equation}
	\begin{gathered}
		\begin{bmatrix}
			{I} & -{D}_K \\ 0 & {I}
...
\begin{proof}
	...
	\end{proof}
	Домножим с правой и левой стороны левую часть (\ref{eq: S procedure}) на матрицы $diag({Q}_{CL}\T, {I}, {I})$ и $diag({Q}_{CL}, {I}, {I})$ для линейной независимости в переменных:
	\fixme{умножаем ли мы и правую часть?}
	\begin{align}
		\begin{bmatrix}
...
\end{proof}
Целевой функцией будет являться минимизация параметра  $\alpha$ \fixme{добавить обоснование целевой функции}. Оптимизационная задача будет выглядеть следующим образом:
%
\begin{equation}
	\label{eq:thm2_OCP}
	\begin{aligned}
...
\section{Пример применения метода нахождения оптимального робастного управления для системы с аддитивной неопределённостью и динамической обратной связью}\label{sec:ch5/sect2}
...
Сначала решим следующую ЗОУ с целевой функцией \fixme{что делает функция посмотреть в коде}:
%
\begin{equation}
	\label{eq:thm7_OCP}
	\begin{aligned}
...
\end{equation}
%
На рисунке \ref{fig:output_cost} изображён график зависимости значения целевой функции от значения параметра $\alpha$. Данный график показывает, что у ЗОУ есть решения и можно выбрать оптимальное значение параметра $\alpha$, включив его в целевую функцию как описано в проблеме \eqref{eq:thm2_OCP}.

На рисунке показаны графики зависимости вектора состояния при решении проблемы \eqref{eq:thm2_OCP} для заданных случайных начальных условий и временном промежутке в секунд.

\begin{figure}[ht]
	\centerfloat{
		\includegraphics[scale=1.2]{images/output_cost.JPG}
	}
	\caption{Целевая функция для ЗОУ \eqref{eq:thm7_OCP}, решённой для плоского четвероногого робота, построенная относительно параметра $\alpha$; вертикальная ось масштабирована логарифмически, обе оси без единиц.} \label{fig:output_cost}
\end{figure}